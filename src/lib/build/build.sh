...
pkgctl_build() {
	...
	for path in "${paths[@]}"; do
		...
		if [[ -n ${PKGVER} ]]; then
			...
			grep -Eqm1 "^pkgver=${pkgver}$" PKGBUILD || die "Non-standard pkgver declaration"
			sed -E "s|^(pkgver=)${pkgver}$|\1${PKGVER}|g" -i PKGBUILD
		fi

		# update pkgrel
		if [[ -n ${PKGREL} ]]; then
			msg "Bumping pkgrel to ${PKGREL}"
			grep -Eqm1 "^pkgrel=${pkgrel}$" PKGBUILD || die "Non-standard pkgrel declaration"
			sed -E "s|^(pkgrel=)${pkgrel}$|\1${PKGREL}|g" -i PKGBUILD
		fi
