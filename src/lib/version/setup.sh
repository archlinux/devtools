...
pkgctl_version_setup_usage() {
	...
	cat <<- _EOF_
		...
		OPTIONS
		    ...
		    --none                 Setup a blank config for packages without remote sources
...
pkgctl_version_setup() {
	...
	for path in "${pkgbases[@]}"; do
		...
		pushd "${path}" >/dev/null
		if nvchecker_setup "${path}" "${force}" "${prefer_platform_api}" "${override_url}" "${none}"; then
			checks+=("${path}")
		else
			ret=1
		fi
