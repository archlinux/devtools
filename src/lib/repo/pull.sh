...
git_rewind_stash() {
	...
	if [[ -n "${maybe_id}" ]]; then
		git -C "${path}" stash apply --quiet "${maybe_id}"
	fi
}

pkgctl_repo_pull() {
	# options
	local paths path pkgbase branch jobs remote stash_entry
	jobs=$(nproc)
	...
	for path in "${paths[@]}"; do
		...
		if (( DISCARD_CHANGES )) || (( AUTOSTASH )); then
			# save current state
			# if autostash is not set this will get cleaned up after some time by git gc
			stash_entry=$(git -C "${path}" stash create)

			# discard any local modifications
			git -C "${path}" checkout --force -- .
		fi

		remote=$(git -C "${path}" config "branch.${branch}.remote")
		if [[ -z "${remote}" ]]; then
			msg_error "  No upstream tracking branch configured: ${pkgbase}"
			git_rewind_stash "${path}" "${stash_entry}"
			continue
		fi
